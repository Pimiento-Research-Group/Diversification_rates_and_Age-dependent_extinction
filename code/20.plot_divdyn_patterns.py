...
Description:
1. Plotting of a skyline plot of the patterns obtained using the DivDyn package
2. Plotting of cumulative net diversification from DivDyn and PyRate
"""
...
# 1. Skyline plot of DivDyn estimated rates
...
# 2. Plotting cumulative net diversification

d = "/Users/kristinakocakova/" # or "/Volumes/External_memory/"

# DivDyn rates

df_d = pd.read_csv(d + "Dropbox/Kristina_PhD_K_version/Kristina_files/Analyses/PyRate/PyRate_Analysis/inputs/2025/June/div_dyn_rates.txt", sep = "\t")
...
n = 0
for i in df_d["net_div_mean"]:
    n += i
    net_div_c.append(n)

df_d["net_div_c"] = net_div_c

# PyRate rates

df_p = pd.read_excel(d + "Dropbox/Kristina_PhD_K_version/Kristina_files/Analyses/PyRate/PyRate_Analysis/outputs/2025/June/species/rates.xlsx")

df_p = df_p.sort_values("Time_e")

df_p["net_div"] = df_p["Rate_s"] - df_p["Rate_e"]

net_div_c = []

n = 0
for i in df_p["net_div"]:
    n += i
    net_div_c.append(n)

df_p["net_div_c"] = net_div_c
...
fig, ax = plt.subplot_mosaic(
    ...
    DD
    """,
    figsize=(6.69291, 6), gridspec_kw={"height_ratios": [3, 3, 0.5, 0.25]}, sharex = True
)

fig.subplots_adjust(hspace=0.05)

sns.lineplot(data = df_p, x = "Time_e", y = "net_div_c", color = "#4f5450", ax = ax["A"])
ax["A"].set_ylabel("Cumulative net diversification", fontsize=7)

ax["B"].step(x = df_d["max_ma"], y = df_d["net_div_c"], color = "#4f5450")
ax["B"].set_ylabel("Per capita\ncumulative net diversification", fontsize=7)
...
ax["C"].bar(stage_chrono, bar_vals_stage, width=width_stage, color=cols_stage, edgecolor="none")
ax["C"].bar_label(ax["C"].containers[0], labels=stage_chrono_names, padding=-15, fontsize=6, rotation=90)
ax["C"].set_xlim(xmin=-146)
ax["C"].set_ylim(ymin=0, ymax=0.4)
...
ax["D"].bar(epoch_chrono, bar_vals, width=width, color=cols, edgecolor="none")
ax["D"].bar_label(ax["D"].containers[0], labels=epoch_chrono_names, padding=-10, fontsize=6)
ax["D"].set_xlim(xmin=-146)
ax["D"].set_ylim(ymin=0, ymax=0.25)
ax["D"].set_yticks(ticks=[0], labels=[""])

ax["D"].set_xticks(ticks=[-100.5, -66, -56, -33.9, -23.03, -5.333],
                   labels=["", "", "", "", "", ""])
ax["D"].set_xlabel("Time (Ma)", fontsize=7)
ax["D"].axis("off")
ax["D"].set_visible(True)
